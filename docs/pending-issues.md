...
### APPR-001 操作提案没有专用 SSE 事件

- **现状**：T22 的提案出现在会话快照（`actionProposals`）中，前端依赖 Agent Run 终态事件触发的快照重拉来展示新提案；spec 第 79 行描述 SSE 应推送"提案"事件，但事件契约 v1 没有 proposal 类型。
- **为什么暂缓**：T22 的 AC 只要求提案随界面展示且状态明确；提案由 Python 在 COMPLETED 之前提交，终态快照重拉必然能看到；加事件类型需要四处同步白名单（契约、Java 枚举、DB CHECK、流转矩阵）。
- **升级触发条件**：T23 引入审批/拒绝后，提案状态变更不再伴随 Agent Run 终态事件，需要独立的事件流；或前端要求提案实时出现（不等终态）。
- **建议方案**：事件契约 v1 增加 `agent.run.proposal` 事件类型（复用 finding 事件的四层白名单同步流程），Java 在 grant 端点创建草案与确认/放弃时落事件。
- **验收标准**：提案创建与状态变更无需等待终态即可在 SSE 流上观察到；事件可重放且去重。
